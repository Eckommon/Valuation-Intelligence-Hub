...
and M24 extend the same approval lock to debt, diluted shares, and governed WACC
while preserving older proposal behavior. No path overwrites a Draft file or
...
from valuation_hub.wacc_draft_binding import validate_binding_proposal_any
...
def _wacc_context(proposal: dict[str, Any]) -> dict[str, Any]:
    context=proposal.get("baseline_context",{}).get("wacc_assumption")
    if not isinstance(context,dict):
        raise CaseServiceError("WACC binding context missing / WACC 바인딩 context 누락")
    return context


def _validate_wacc_targets(proposal:dict[str,Any],normalized_draft:dict[str,Any])->None:
    context=_wacc_context(proposal)
    names=context.get("scenario_names")
    if not isinstance(names,list) or not names:
        raise CaseServiceError("WACC scenario target set missing / WACC 시나리오 대상집합 누락")
    actual=sorted(normalized_draft["equity"]["scenarios"])
    if sorted(names)!=actual:
        raise CaseServiceError("WACC scenario target set must exactly match Draft scenarios / WACC 대상 시나리오는 Draft 시나리오와 정확히 일치해야 함")
...
def build_binding_approval(
    ...
    *,
    ...
    if normalized_draft["model"] != "equity_fcff":
        raise CaseServiceError("M17/M20/M23/M24 requires equity_fcff Draft / M17/M20/M23/M24는 equity_fcff Draft 전용")
...
def _get_field(draft: dict[str, Any], field: str, context:dict[str,Any]|None=None) -> Any:
    ...
    if field == "scenario.wacc":
        if not isinstance(context,dict) or not isinstance(context.get("scenario_names"),list):
            raise CaseServiceError("WACC context required for Draft read / Draft WACC 읽기에 context 필요")
        keys=_scenario_key_map(draft)
        names=context["scenario_names"]
        if set(names)!=set(keys):
            raise CaseServiceError("WACC target scenarios mismatch Draft / WACC 대상 시나리오와 Draft 불일치")
        return {name:draft["equity"]["scenarios"][keys[name]]["wacc"] for name in sorted(names)}
...
def _set_field(draft: dict[str, Any], field: str, value: Any, context:dict[str,Any]|None=None) -> None:
    ...
    if field == "scenario.wacc":
        if not isinstance(context,dict) or not isinstance(value,dict):
            raise CaseServiceError("WACC context/value mapping required / WACC context·값 mapping 필요")
        keys=_scenario_key_map(draft)
        if set(value)!=set(keys) or set(context.get("scenario_names",[]))!=set(keys):
            raise CaseServiceError("WACC write target set mismatch / WACC 쓰기 대상집합 불일치")
        for name,new_value in value.items():
            draft["equity"]["scenarios"][keys[name]]["wacc"]=new_value
        return
    raise CaseServiceError(f"unsupported binding apply field / 미지원 바인딩 적용필드: {field}")
...
def apply_binding_approval(proposal: dict[str, Any], draft: dict[str, Any], approval: dict[str, Any]) -> dict[str, Any]:
    validate_binding_approval(approval, proposal, draft)
    before = copy.deepcopy(draft); after = copy.deepcopy(draft)
    ...
    for field in approval["approved_fields"]:
        ...
        old = _get_field(after, field, context); new = _target_value(field,context)
...
def validate_bound_draft_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    for diff in diffs:
        ...
        field=diff["field"];decision=direct[field];context=_binding_context(proposal,decision)
