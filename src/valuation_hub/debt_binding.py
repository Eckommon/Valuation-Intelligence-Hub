...
def build_debt_date_assertion(
    debt: dict[str, Any],
    *,
    reviewer: str,
    approved_at: str,
    asserted_period_end: str,
    review_basis: str,
) -> dict[str, Any]:
    """Build a human assertion for REPORT_STAGE_ONLY debt; never synthesize a date."""
    ...
    assertion = {
        "schema_version": DATE_ASSERTION_SCHEMA,
        "status": DATE_ASSERTION_STATUS,
        "canonical": False,
        "reviewer": reviewer.strip(),
        "approved_at": approved.isoformat(),
        "source_debt_sha256": debt["debt_sha256"],
        "target_identity": copy.deepcopy(debt["entity"]),
        "source_period_identity": _period_identity(debt),
        "asserted_period_end": asserted.isoformat(),
...
def validate_debt_date_assertion(assertion: dict[str, Any], debt: dict[str, Any]) -> dict[str, Any]:
    ...
    basis = assertion.get("review_basis")
...
def build_debt_binding_context(
    debt: dict[str, Any],
    *,
    as_of: str,
    max_age_days: int = 550,
    date_assertion: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Evaluate M19 debt for future Draft binding without mutating any Draft."""
    ...
    freshness = {"status": FRESH if age <= max_age_days else STALE_BLOCKED, "age_days": age, "max_age_days": max_age_days}
    ...
    context = {
        ...
        "value": value,
        "unit": debt["unit"],
        "entity": copy.deepcopy(debt["entity"]),
        "source_period": copy.deepcopy(debt["period"]),
        "resolved_period_end": resolved_end,
...
def validate_debt_binding_context(
    ...
    *,
    ...
    _parse_date(context.get("resolved_period_end"), "resolved_period_end")
    resolution = context.get("date_resolution")
    freshness = context.get("freshness")
    eligibility = context.get("binding_eligibility")
    ...
    if not isinstance(freshness, dict) or freshness.get("status") not in {FRESH, STALE_BLOCKED} or not isinstance(freshness.get("age_days"), int) or not isinstance(freshness.get("max_age_days"), int):
        raise CaseServiceError("debt freshness invalid / debt 최신성 오류")
    expected_eligible = freshness["status"] == FRESH
    ...
    if resolution["method"] == "SOURCE_EXACT":
        ...
        if not isinstance(assertion_sha, str) or not SHA256_RE.fullmatch(assertion_sha):
    ...
    if debt is not None:
        ...
        resolved_end, expected_resolution = _resolve_period_end(debt, date_assertion)
        if context.get("resolved_period_end") != resolved_end or context.get("date_resolution") != expected_resolution:
    ...
    return {"status": "PASS_DEBT_BINDING_CONTEXT_VALIDATION", "canonical": False, "context_sha256": expected, "eligible": expected_eligible, "freshness_status": freshness["status"]}
