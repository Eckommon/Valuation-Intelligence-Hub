...
def _debt_decision(context:dict[str,Any])->dict[str,Any]:
    ...
    return {
        "field":"equity.debt","state":state,"rationale":rationale,
...
def build_binding_proposal_with_debt(
    ...
    *,as_of:str,
    ...
    for item in base["draft_input_matrix"]:
        matrix.append(_debt_decision(debt_context) if item["field"]=="equity.debt" else copy.deepcopy(item))
    baseline=copy.deepcopy(base["baseline_context"])
    baseline["interest_bearing_debt"]={
        "metric":"interest_bearing_debt","value":debt_context["value"],"unit":debt_context["unit"],"class":"DERIVED_FACT",
        "entity":copy.deepcopy(debt_context["entity"]),"source_period":copy.deepcopy(debt_context["source_period"]),
        "resolved_period_end":debt_context["resolved_period_end"],"freshness":copy.deepcopy(debt_context["freshness"]),
        "context_sha256":debt_context["context_sha256"],"source_debt_sha256":debt_context["source_debt_sha256"],
        "date_resolution":copy.deepcopy(debt_context["date_resolution"]),
    }
    proposal={
        ...
        "draft_input_matrix":matrix,"completeness":_counts(matrix),
...
def validate_binding_proposal_v2(proposal:dict[str,Any])->dict[str,Any]:
    ...
    if not isinstance(debt,dict) or debt.get("metric")!="interest_bearing_debt" or debt.get("class")!="DERIVED_FACT":
        raise CaseServiceError("debt binding context missing from proposal / proposal debt context 누락")
    context_sha=debt.get("context_sha256")
    if not isinstance(context_sha,str) or proposal.get("source_debt_context_sha256")!=context_sha:
        raise CaseServiceError("debt context SHA lineage mismatch / debt context SHA lineage 불일치")
    identity=proposal["identity"]
    if debt.get("entity")!={"id":identity.get("entity_id"),"financial_scope":identity.get("financial_scope")} or debt.get("unit")!=identity.get("monetary_unit"):
    ...
    if decision.get("source_metric")!="interest_bearing_debt" or decision.get("source_class")!="DERIVED_FACT" or decision.get("source_context_sha256")!=context_sha or decision.get("source_debt_sha256")!=debt.get("source_debt_sha256") or decision.get("date_assertion_sha256")!=debt.get("date_resolution",{}).get("date_assertion_sha256"):
        raise CaseServiceError("debt DIRECT_BIND lineage mismatch / debt DIRECT_BIND lineage 불일치")
    fresh=debt.get("freshness",{}).get("status")
    expected_state=DIRECT_BIND if fresh==FRESH else STALE_BLOCKED
    if decision.get("state")!=expected_state or decision.get("source_freshness")!=debt.get("freshness"):
        raise CaseServiceError("debt binding freshness classification mismatch / debt 바인딩 최신성 판정 불일치")
