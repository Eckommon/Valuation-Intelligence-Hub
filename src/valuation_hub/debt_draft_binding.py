...
def _counts(matrix: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {"material_field_count": len(MATERIAL_FIELDS), "classified_field_count": len(matrix), "state_counts": state_counts, "direct_bind_count": state_counts.get(DIRECT_BIND, 0), "unresolved_count": sum(count for state, count in state_counts.items() if state != DIRECT_BIND)}


def _debt_projection(context: dict[str, Any]) -> dict[str, Any]:
    return {
        "metric": "interest_bearing_debt", "value": context["value"], "unit": context["unit"], "class": "DERIVED_FACT",
        "entity": copy.deepcopy(context["entity"]), "source_period": copy.deepcopy(context["source_period"]),
        "resolved_period_end": context["resolved_period_end"], "freshness": copy.deepcopy(context["freshness"]),
        "context_sha256": context["context_sha256"], "source_debt_sha256": context["source_debt_sha256"],
...
def _debt_decision(context: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "field": "equity.debt", "state": DIRECT_BIND if eligible else STALE_BLOCKED,
        "rationale": "complete reviewed fresh M19 interest-bearing debt / 완전·검토완료·최신 M19 이자부채" if eligible else "complete reviewed debt exceeds freshness policy / 완전·검토완료 debt가 최신성 정책 초과",
        "source_metric": "interest_bearing_debt", "source_class": "DERIVED_FACT",
        "source_freshness": copy.deepcopy(context["freshness"]), "source_context_sha256": context["context_sha256"],
        "source_debt_sha256": context["source_debt_sha256"], "date_assertion_sha256": context["date_resolution"].get("date_assertion_sha256"),
    }


def build_binding_proposal_with_debt(observations: list[dict[str, Any]], debt_context: dict[str, Any], *, as_of: str, max_age_days: int = 550) -> dict[str, Any]:
    base = build_binding_proposal(observations, as_of=as_of, max_age_days=max_age_days)
    validate_debt_binding_context(debt_context)
    if debt_context.get("policy", {}).get("as_of") != as_of or debt_context.get("policy", {}).get("max_age_days") != max_age_days:
        raise CaseServiceError("debt context freshness policy must match proposal policy / debt context 최신성 정책과 proposal 정책 불일치")
    ...
    matrix = [_debt_decision(debt_context) if item["field"] == "equity.debt" else copy.deepcopy(item) for item in base["draft_input_matrix"]]
    baseline = copy.deepcopy(base["baseline_context"]); baseline["interest_bearing_debt"] = _debt_projection(debt_context)
    proposal = {
        "schema_version": SCHEMA_VERSION_V2, "status": STATUS, "canonical": False, "target": copy.deepcopy(base["target"]),
        "policy": {"version": POLICY_VERSION_V2, "as_of": as_of, "max_age_days": max_age_days, "direct_bind_requires": ["SEMANTIC_EXACT", "REVIEWED_DERIVED_FACT", "FRESH", "PERIOD_COMPATIBLE", "DEBT_COMPLETE_CORE_COMPONENTS"]},
        "identity": copy.deepcopy(identity), "baseline_context": baseline, "conflicts": copy.deepcopy(base["conflicts"]),
        "draft_input_matrix": matrix, "completeness": _counts(matrix), "source_observation_sha256": copy.deepcopy(base["source_observation_sha256"]),
        "source_debt_context_sha256": debt_context["context_sha256"], "debt_binding_context": copy.deepcopy(debt_context),
        ...
        "warning_ko": "Debt-aware 제안 전용입니다. Draft를 변경하거나 근거 권위를 승격하지 않습니다.", "proposal_sha256": "",
    }
...
def validate_binding_proposal_v2(proposal: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(base, dict): raise CaseServiceError("base v0.1 proposal missing / base v0.1 proposal 누락")
    ...
    if not isinstance(full_context, dict): raise CaseServiceError("full debt binding context missing / 전체 debt 바인딩 context 누락")
    validate_debt_binding_context(full_context)
    context_policy = full_context.get("policy", {})
    if context_policy.get("as_of") != policy.get("as_of") or context_policy.get("max_age_days") != policy.get("max_age_days"):
        raise CaseServiceError("debt context/proposal freshness policy mismatch / debt context·proposal 최신성 정책 불일치")
    ...
    if not isinstance(baseline, dict): raise CaseServiceError("debt-aware baseline context invalid / debt-aware baseline context 오류")
    if baseline.get("interest_bearing_debt") != _debt_projection(full_context):
    ...
    base_by = {item["field"]: item for item in base["draft_input_matrix"]}; by = {item["field"]: item for item in matrix}
    ...
    if by["equity.debt"] != _debt_decision(full_context):
    ...
    return {"status": "PASS_DEBT_AWARE_BINDING_PROPOSAL_VALIDATION", "canonical": False, "proposal_sha256": expected, "direct_bind_count": proposal["completeness"]["direct_bind_count"], "unresolved_count": proposal["completeness"]["unresolved_count"]}
