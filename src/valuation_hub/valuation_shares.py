...
Bridge validation embeds and revalidates full source objects instead of trusting
summary hashes alone.
"""
...
def extract_sec_current_common_shares_candidate(
    snapshot: dict[str, Any], *, period_end: str, form: str | None = None
) -> dict[str, Any]:
    ...
    for raw in series:
        if not isinstance(raw, dict) or raw.get("end") != end or raw.get("start") is not None:
            continue
    ...
    chosen = sorted(
        top,
        key=lambda x: (str(x.get("accn", "")), str(x.get("form", "")), str(x.get("frame", ""))),
        reverse=True,
    )[0]
    ...
    candidate = {
        ...
        "source": {
            ...
            "source_type": "official_edgar_companyfacts_api",
            "tier_proposal": "A",
        ...
        "selection": {
            "rule": "EXACT_INSTANT_THEN_LATEST_FILED",
            "requested_end": end,
...
def validate_current_common_shares_candidate(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        candidate.get("metric") != "current_common_shares"
        or candidate.get("unit") != "shares"
        or candidate.get("taxonomy") != "dei"
        or candidate.get("concept") != CONCEPT
    ):
        raise CaseServiceError("current-share candidate semantics invalid / 현재주식수 candidate 의미 오류")
    _source_class(candidate.get("class"))
    _num(candidate.get("value"), "current common shares", positive=True)
    ...
    if not all(isinstance(x, dict) for x in (entity, period, filing, source, selection)):
        raise CaseServiceError("current-share candidate provenance incomplete / 현재주식수 candidate 출처정보 불완전")
    if not isinstance(entity.get("id"), str) or not entity["id"].startswith("SEC_CIK:") or entity.get("financial_scope") != "AS_REPORTED":
        raise CaseServiceError("current-share candidate entity invalid / 현재주식수 candidate entity 오류")
    if period.get("kind") != "INSTANT" or period.get("start") is not None or period.get("date_precision") != "EXACT":
...
def normalize_current_common_shares_candidate(candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    result = {
        ...
        "canonical": False,
        "class": _source_class(candidate["class"]),
        "metric": "current_common_shares",
        ...
        "lineage": {
            ...
            "source_class": candidate["class"],
            "source_snapshot_sha256": candidate["source"]["snapshot_sha256"],
            "source_body_sha256": candidate["source"]["body_sha256"],
            "filing_identity": copy.deepcopy(candidate["filing"]),
            "source_detail": {"taxonomy": "dei", "concept": CONCEPT},
        },
        "semantic_boundary": {"current_common_shares_only": True, "fully_diluted_shares": False},
        "observation_sha256": "",
    }
...
def validate_current_common_shares_observation(obs: dict[str, Any]) -> dict[str, Any]:
    if (
        ...
        or obs.get("schema_version") != OBSERVATION_SCHEMA
        or obs.get("status") != OBSERVATION_STATUS
...
def _freshness(end: str, as_of: str, max_age_days: int) -> dict[str, Any]:
    end_date = date.fromisoformat(_iso(end, "period.end"))
    ...
    if age < 0:
        raise CaseServiceError("share instant is after as_of / 주식수 시점이 as_of 이후")
...
def build_valuation_share_base_context(obs: dict[str, Any], *, as_of: str, max_age_days: int = 180) -> dict[str, Any]:
    ...
    result = {
        ...
        "value": obs["value"],
        "unit": "shares",
        "entity": copy.deepcopy(obs["entity"]),
        "source_period": copy.deepcopy(obs["period"]),
        "policy": {"version": "VALUATION_SHARE_BASE_V01", "as_of": as_of, "max_age_days": max_age_days},
        "freshness": freshness,
        "semantic_boundary": {"current_common_shares_base": True, "fully_diluted_shares": False, "direct_bind_to_diluted_shares": False},
        "source_observation_sha256": obs["observation_sha256"],
        "context_sha256": "",
    }
    result["context_sha256"] = _sha(_without(result, "context_sha256"))
    validate_valuation_share_base_context(result, observation=obs)
    return result


def validate_valuation_share_base_context(
    context: dict[str, Any], *, observation: dict[str, Any] | None = None
) -> dict[str, Any]:
    if (
        not isinstance(context, dict)
        or context.get("schema_version") != CONTEXT_SCHEMA
        or context.get("status") != CONTEXT_STATUS
        or context.get("canonical") is not False
    ):
    ...
    period = context.get("source_period")
    ...
    if observation is not None:
        ...
        if (
            observation["class"] != "NORMALIZED_FACT"
            or context["source_observation_sha256"] != observation["observation_sha256"]
            or context["entity"] != observation["entity"]
            or context["value"] != observation["value"]
            or context["source_period"] != observation["period"]
        ):
...
def build_dilution_adjustment(
    *, adjustment_id: str, category: str, shares: int | float, source_sha256: str, source_description: str
) -> dict[str, Any]:
    ...
    adjustment = {
        "schema_version": ADJUSTMENT_SCHEMA,
        "status": ADJUSTMENT_STATUS,
        "canonical": False,
        "class": "NORMALIZED_FACT_CANDIDATE",
        "adjustment_id": adjustment_id.strip(),
        "category": category,
        "shares": _num(shares, "adjustment shares"),
        "source_sha256": source_sha256,
        "source_description": source_description.strip(),
        "semantic_boundary": {"explicit_only": True, "historical_factor_generated": False},
        "adjustment_sha256": "",
    }
...
def validate_dilution_adjustment(adjustment: dict[str, Any]) -> dict[str, Any]:
    if (
        ...
        or adjustment.get("schema_version") != ADJUSTMENT_SCHEMA
        or adjustment.get("status") != ADJUSTMENT_STATUS
        or adjustment.get("canonical") is not False
    ):
...
def build_dilution_coverage_assertion(
    ...
    *,
    ...
    if not isinstance(adjustments, list):
        raise CaseServiceError("adjustments list required / adjustments 배열 필요")
    ...
    assertion = {
        ...
        "adjustment_sha256": sorted(item["adjustment_sha256"] for item in adjustments),
        "reviewed_categories": list(ADJUSTMENT_CATEGORIES),
        "coverage_basis": coverage_basis.strip(),
        "assertion_sha256": "",
    }
    assertion["assertion_sha256"] = _sha(_without(assertion, "assertion_sha256"))
    validate_dilution_coverage_assertion(assertion, base_context, adjustments)
    return assertion


def validate_dilution_coverage_assertion(
    assertion: dict[str, Any], base_context: dict[str, Any], adjustments: list[dict[str, Any]]
) -> dict[str, Any]:
    validate_valuation_share_base_context(base_context)
    if (
        not isinstance(assertion, dict)
        or assertion.get("schema_version") != ASSERTION_SCHEMA
        or assertion.get("status") != ASSERTION_STATUS
        or assertion.get("canonical") is not False
        or assertion.get("decision") != COMPLETE
    ):
    ...
    for item in adjustments:
        validate_dilution_adjustment(item)
        if item["class"] != "NORMALIZED_FACT":
            raise CaseServiceError("candidate adjustment cannot be complete coverage / 후보 조정은 완전 coverage 불가")
...
def _reconcile_adjustments(adjustments: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    grouped: dict[str, list[dict[str, Any]]] = {}
    for item in adjustments:
        validate_dilution_adjustment(item)
        grouped.setdefault(item["adjustment_id"], []).append(item)
    ...
    for adjustment_id, group in sorted(grouped.items()):
        ...
        if len(hashes) > 1:
            ...
            selected.append(copy.deepcopy(group[0]))
    return selected, conflicts


def build_diluted_share_bridge(
    base_context: dict[str, Any],
    adjustments: list[dict[str, Any]],
    *,
    ...
    if conflicts:
        if coverage_assertion is not None:
            raise CaseServiceError("coverage assertion cannot override adjustment conflict / coverage 승인은 조정충돌을 덮을 수 없음")
        ...
        if coverage_assertion is not None:
            ...
            assertion_copy = copy.deepcopy(coverage_assertion)
        else:
    ...
    result = {
        ...
        "canonical": False,
        "class": "DERIVED_FACT" if eligible else "DERIVED_FACT_CANDIDATE",
        "entity": copy.deepcopy(base_context["entity"]),
        "unit": "shares",
        "as_of": base_context["policy"]["as_of"],
        "base_context": copy.deepcopy(base_context),
        "input_adjustments": copy.deepcopy(adjustments),
        "selected_adjustments": selected,
        "conflicts": conflicts,
        "coverage": {"status": coverage_status, "missing_as_zero": False},
        "coverage_assertion": assertion_copy,
        "candidate_fully_diluted_shares": candidate_value,
        "historical_reference": historical_wrapper,
        "binding_eligibility": {
            "eligible_for_future_direct_bind": eligible,
            "reason": "COMPLETE_REVIEWED_FRESH" if eligible else coverage_status,
        },
        "semantic_boundary": {
            "current_common_shares_equal_fully_diluted": False,
            "historical_factor_auto_adjustment": False,
            "missing_adjustments_as_zero": False,
        },
        "bridge_sha256": "",
    }
    result["bridge_sha256"] = _sha(_without(result, "bridge_sha256"))
    validate_diluted_share_bridge(result)
    return result
...
def validate_diluted_share_bridge(bridge: dict[str, Any]) -> dict[str, Any]:
    if (
        ...
        or bridge.get("schema_version") != BRIDGE_SCHEMA
        or bridge.get("status") != BRIDGE_STATUS
        or bridge.get("canonical") is not False
    ):
    ...
    base_context = bridge.get("base_context")
    if not isinstance(base_context, dict):
        raise CaseServiceError("diluted-share bridge base context missing / 희석주식 bridge base context 누락")
    validate_valuation_share_base_context(base_context)
    if bridge.get("entity") != base_context.get("entity") or bridge.get("as_of") != base_context.get("policy", {}).get("as_of"):
        raise CaseServiceError("diluted-share bridge base identity/as_of mismatch / 희석주식 bridge base 식별·as_of 불일치")

    inputs = bridge.get("input_adjustments")
    selected = bridge.get("selected_adjustments")
    conflicts = bridge.get("conflicts")
    if not isinstance(inputs, list) or not isinstance(selected, list) or not isinstance(conflicts, list):
        raise CaseServiceError("diluted-share bridge adjustment structure invalid / 희석주식 bridge 조정 구조 오류")
    expected_selected, expected_conflicts = _reconcile_adjustments(inputs)
    if selected != expected_selected or conflicts != expected_conflicts:
        raise CaseServiceError("diluted-share bridge reconciliation mismatch / 희석주식 bridge 조정 reconciliation 불일치")
    ...
    if expected_conflicts:
        ...
        if assertion is not None:
            raise CaseServiceError("conflict bridge cannot carry coverage assertion / 충돌 bridge coverage 승인 불가")
    else:
        if assertion is not None:
            if not isinstance(assertion, dict):
                raise CaseServiceError("coverage assertion invalid / coverage 승인 오류")
            validate_dilution_coverage_assertion(assertion, base_context, expected_selected)
            expected_status = COMPLETE
        else:
            expected_status = BASE_ONLY if not expected_selected else PARTIAL
        expected_value = base_context["value"] + sum(item["shares"] for item in expected_selected)

    coverage = bridge.get("coverage")
    if coverage != {"status": expected_status, "missing_as_zero": False}:
        raise CaseServiceError("diluted-share bridge coverage mismatch / 희석주식 bridge coverage 불일치")
    if expected_value is None:
        if bridge.get("candidate_fully_diluted_shares") is not None:
            raise CaseServiceError("conflict bridge must hide candidate total / 충돌 bridge candidate 합계 비공개 필요")
    else:
    ...
    if historical is not None:
        if not isinstance(historical, dict) or historical.get("reference_only") is not True or historical.get("auto_adjustment_created") is not False or not isinstance(historical.get("evidence"), dict):
            raise CaseServiceError("historical dilution reference boundary invalid / 역사적 희석도 참조경계 오류")
        validate_historical_dilution(historical["evidence"])

    all_reviewed = all(item["class"] == "NORMALIZED_FACT" for item in expected_selected)
    eligible = expected_status == COMPLETE and base_context["freshness"]["status"] == FRESH and all_reviewed
    expected_binding = {
        "eligible_for_future_direct_bind": eligible,
        "reason": "COMPLETE_REVIEWED_FRESH" if eligible else expected_status,
    }
    ...
    if bridge.get("semantic_boundary") != {
        "current_common_shares_equal_fully_diluted": False,
        "historical_factor_auto_adjustment": False,
        "missing_adjustments_as_zero": False,
    }:
    ...
    return {
        "status": "PASS_DILUTED_SHARE_BRIDGE_VALIDATION",
        "bridge_sha256": expected,
