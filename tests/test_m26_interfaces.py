...
from valuation_hub import cli_entry_m26, cli_entry_m27, cli_entry_m28, cli_entry_m29, cli_entry_m30, cli_entry_m30r1, cli_entry_m30r2, cli_entry_m30r3, cli_entry_m30r4, cli_entry_m30r5
...
def _scenarios() -> list[dict]:
    return [{"scenario_name": "BASE", "rationale": "Explicit integrated forecast for interface test.", "years": [{"year": 2027, "revenue": 2000.0, "ebit_margin": 0.10, "tax_rate": 0.25, "depreciation_amortization": 80.0, "capex": 100.0, "delta_nwc": 50.0}]}]


def test_m26_cli_builds_forecast_candidate(tmp_path: Path, capsys) -> None:
    path = tmp_path / "scenarios.json"; path.write_text(json.dumps(_scenarios()), encoding="utf-8")
    rc = cli_entry_m26.main(["forecast-candidate-build", str(path), "--entity-id", "DART_CORP:00126380", "--financial-scope", "CFS", "--capital-currency", "KRW", "--as-of", "2026-09-12"])
    assert rc == 0
...
def test_m26_cli_delegates_older_commands_unchanged(monkeypatch) -> None:
    seen: list[list[str]] = []
    def prior(argv: list[str]) -> int: seen.append(argv); return 73
